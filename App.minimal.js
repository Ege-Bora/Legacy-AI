...
import { View, Text, TouchableOpacity, AppRegistry, Alert, ScrollView } from 'react-native';
  ...
  const pollTranscriptionResult = async (jobId, recordingId) => {
    try {
      ...
      const pollInterval = setInterval(async () => {
        ...
        try {
          const response = await fetch(`http://192.168.1.155:8080/memos/jobs/${jobId}/status`, {
            method: 'GET',
            headers: {
              'Content-Type': 'application/json',
            },
